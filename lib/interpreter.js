...
function interpret (program, functionApplication) {
    ...
    // reset the stack, heap and global variables
    stackAddress = 0;
    heapAddress = 0;
    stack = [];
    heap = [];
    ...
    if (program[0] === 'Export') {
        ...
        for (let i = 0; i < program[1].length; i = i + 1) {
            if (program[1][i] !== functionApplication[1][1]) {
                throw new Error('Function was no exported by module.');
            }
        }
        return interpretContent(program[2], functionApplication);
    } else {
        // TODO: throw error
        return interpretContent(program, functionApplication);
...
function interpretContent (content, functionApplication) {
    ...
    if (content[0] === 'New') {
        ...
        address = interpretExpression(content[2], globalVariablesStore, []);
        // point new variable to address
        globalVariablesStore[content[1]] = address;
        // evaluate the rest of the body with the new variable
        let newAddress = interpretContent(content[3], functionApplication);
        ...
        address = interpretFunctions(content, 0, functionApplication, []);
    }
    return address;
}
...
function interpretBody (body, variableStore, functionStore) {
    ...
    switch(body[0]) {
        case 'Sequence':
            ...
            const stmtAddress = (body[1][0] === 'Let' || body[1][0] === 'Break') ? interpretBody(body[1], variableStore, functionStore): interpretStatement(body[1], variableStore, functionStore);
        ...
        case 'New': case 'Let':
            // interpret the expression that signifies the value of the new variable
            const newAddress = interpretExpression(body[2], variableStore, functionStore);
            // point new variable to address
            variableStore[body[1]] = newAddress;
            // evaluate the rest of the body with the new variable
            return interpretBody(body[3], variableStore, functionStore);
        case 'Loop':
            // check the condition
        ...
        case 'Break':
            ...
            return emptyAddress;
        default:
            return interpretExpression(body, variableStore, functionStore);
    }
}
        ...
        case 'Assign':
            ...
            if (statement[1][0] === 'Identifier') {
                ...
                if (statement[1][0] === 'Index') {
                    ...
                    stack[arrAddress][stack[indexAddress]] = stack[interpretExpression(statement[2], variableStore, functionStore)];
                } else {
...
function interpretDereference (dereference, variableStore, functionStore) {
    ...
    if (dereference[1][0] === 'Index') {
        ...
        result = stack[idAddress][stack[indexAddress]];
    } else {
