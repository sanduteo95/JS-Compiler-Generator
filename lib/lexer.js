...
const lexerRules = {
    ...
    "(\\(|\\)|\\{|\\}|\\,|\\;|\\=|function|return|if|else|for|break|switch|case|default|module.exports|\\:)": lexeme => {
        return {token: lexeme};
    },
    ...
    // match undefined
    "undefined": () => {
        return {token: "const", value: undefined};
    },
    // match arrays and array functions
    "(\\[|\\])" : lexeme => {
        return {token: lexeme};
    },
    "\\.length": () => {
    ...
    // match type of variable (TODO: differentiate between types)
    ...
    initialiseLexer () {
        ...
        Object.keys(lexerRules).map(stringifiedRegex => {
            ...
            self.lexer.addRule(regex, lexeme => {
                ...
                // save column and row in case the parser can use them
