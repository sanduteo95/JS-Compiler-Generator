...
function evaluateProgram (program, functionApplication) {
    if (program[0] === 'Export') {
        const evaluatedContent = evaluateContent(program[2], functionApplication);
        if(evaluatedContent[0] !== 'Const' && evaluatedContent[0] !== 'Array' && evaluatedContent.length !== 0) {
...
function evaluateContent (content, functionApplication) {
    ...
    if (content[0] === 'New') {
        ...
        globalVariableStore[content[1]] = evaluateExpression(content[2], globalVariableStore);
        // evaluate the rest of the body with the new variable
        return evaluateProgram(content[3], functionApplication);
    } else {
...
function evaluateBody(body, variableStore, functionStore) {
    ...
    switch(body[0]) {
        case 'Sequence':
            ...
            const evaluatedStatement = (body[1][0] === 'Let' || body[1][0] === 'Break') ? evaluateBody(body[1], variableStore, functionStore) : evaluateStatement(body[1], variableStore, functionStore);

            if(evaluatedStatement[0] === 'Error' || evaluatedStatement[0] === 'Const') {
